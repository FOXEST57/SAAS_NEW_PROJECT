...
/**
 * Jeu d'icônes SVG intégré (tracés de style Lucide, 24×24, stroke).
 * Éviter une dépendance externe garde le bundle léger et le rendu cohérent.
 */
const PATHS: Record<string, string> = {
  dashboard:
    '<rect x="3" y="3" width="7" height="9" rx="1"/><rect x="14" y="3" width="7" height="5" rx="1"/><rect x="14" y="12" width="7" height="9" rx="1"/><rect x="3" y="16" width="7" height="5" rx="1"/>',
  cart: '<circle cx="8" cy="21" r="1"/><circle cx="19" cy="21" r="1"/><path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12"/>',
  file: '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v5h6"/><path d="M8 13h8"/><path d="M8 17h5"/>',
  invoice:
    '<path d="M4 2v20l3-2 3 2 3-2 3 2 3-2V2l-3 2-3-2-3 2-3-2Z"/><path d="M8 8h8"/><path d="M8 12h8"/><path d="M8 16h5"/>',
  package:
...
export class IconComponent {
  ...
  private readonly _size = signal<number>(18);
  private readonly _stroke = signal<number>(1.75);
  ...
  set strokeWidth(value: number | string) {
    this._stroke.set(Number(value));
  }

  protected readonly px = this._size.asReadonly();
  protected readonly stroke = this._stroke.asReadonly();

  /**
   * Les tracés proviennent d'une constante interne (aucune saisie utilisateur),
   * ils sont donc sûrs à injecter tels quels.
   */
  protected readonly markup = computed<SafeHtml>(() => {
    const key = this._name();
    return this.sanitizer.bypassSecurityTrustHtml(PATHS[key] ?? PATHS['info']);
  });
}
